...
void CBaseLayer::transferDiffBlob( int i, CDnnBlob* diffBlob, int outputNum )
{
	...
	if(outputs[outputNum] == 1) {
		...
		if(readyOutputDiffs[outputNum] == 0) {
			...
			if( !outputDiffBlobs[outputNum]->Add( diffBlob, 100 + (10000 * outputNum) + (1000000 * i), &strName ) ) {
				printf( "i=%d outputNum=%d layer='%s' input='%s' \n", i, outputNum, GetName(), inputs[i].Name.Ptr() );
			}
			if( myEpoch > 4000 && strcmp( GetName(), "CCnnEltwiseMulLayer" ) == 0 ) {
				printf( "i=%d outputNum=%d layer='%s' input='%s' \n", i, outputNum, GetName(), inputs[i].Name.Ptr() );
			}
		}
	}

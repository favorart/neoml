...
{
	CheckInputs();
	CheckLayerArchitecture( inputDescs.Size() == 2, "layer expects 2 inputs" );
	CheckLayerArchitecture( inputDescs[0].ObjectCount() == inputDescs[1].ObjectCount(),
		"Object count mismatch between inputs" );
	CheckLayerArchitecture( inputDescs[0].ObjectSize() == inputDescs[1].ObjectSize() || inputDescs[1].ObjectSize() == 1,
		"Inputs' object sizes must match or the second one must be equal to 1" );
	CheckLayerArchitecture( !outputDescs.IsEmpty(), "There is nothing connected to this layer's output" );
}

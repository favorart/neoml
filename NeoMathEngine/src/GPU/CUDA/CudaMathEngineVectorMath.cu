	...
	const CIntHandle& resultHandle, int vectorSize)
{
	...
	VectorEltwiseMultiplyKernel<<<blockCount, threadCount>>>
		(GetRaw(firstHandle), GetRaw(secondHandle), GetRaw(resultHandle), vectorSize, 0, 0);
}
	...
	const CFloatHandle& resultHandle, int vectorSize)
{
	...
	VectorEltwiseMultiplyKernel<<<blockCount, threadCount>>>
		(GetRaw(firstHandle), GetRaw(secondHandle), GetRaw(resultHandle), vectorSize, blockCount, threadCount);
}

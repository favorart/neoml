...
{
	//std::cout << "input_init=" << input_init << " max=" << ( stride * ( ( ( filter - 1 ) * dilation + 2 * padding - 1 ) / stride - 1 ) ) << std::endl;
	return std::max( input_init, stride * ( ( ( filter - 1 ) * dilation + 2 * padding - 1 ) / stride - 1 ) );
}
